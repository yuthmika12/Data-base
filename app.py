...
# Replace this with the Web App URL you copied from Step 1
WEBHOOK_URL = "https://script.google.com/macros/s/AKfycbyfiId8n4BgUPp_Rk9byZNK-G7Tplfcsws6xWuIHrJRxDGFYy7Vs3Or2tjtiYbDC5kM/exec"

# Clean CSV public read URL
    ...
    with st.form("add_member_form"):
        ...
        if submit_button:
            ...
            if clean_name == "":
                ...
                # --- SENDING DATA LIVE TO GOOGLE SCRIPT ---
                payload = {
                ...
                try:
                    with st.spinner("Saving member to cloud registry..."):
                        response = requests.post(WEBHOOK_URL, json=payload)
                    
                    if response.status_code == 200:
                        st.success(f"🎉 Successfully registered {clean_name} and saved to Spreadsheet!")
                    
                    else:
                        st.error(f"Failed to connect to spreadsheet pipeline. Status code: {response.status_code}")
                except Exception as e:
                    st.error(f"Network error trying to contact the spreadsheet pipeline: {e}")
